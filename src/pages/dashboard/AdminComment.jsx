...
import {
    FaComment, FaSearch, FaTrash,
    FaUserCircle, FaClock, FaExternalLinkAlt
} from "react-icons/fa";
...
const AdminComments = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            await API.delete(`/admin/posts/${deleteTarget.slug}/comment/${deleteTarget.commentId}`);
            setComments(comments.filter((c) => c.comment._id !== deleteTarget.commentId));
            setTotal(total - 1);
        ...
        <div style={{ backgroundColor: "var(--bg)", minHeight: "100vh" }}>
            <div className="container py-5">
                ...
                <div className="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
                    ...
                    <Link to="/admin/dashboard" className="btn btn-sm fw-semibold"
                ...
                <div className="bg-white rounded shadow-sm p-3 mb-4">
                    <div className="position-relative" style={{ maxWidth: "360px" }}>
                        ...
                        <input type="text" className="form-control" placeholder="Search by comment or name..."
                ...
                {comments.length === 0 && !loading ? (
                    <div className="text-center py-5 bg-white rounded shadow-sm">
                        ...
                        <p style={{ color: "var(--gray)", fontSize: "14px" }}>No comments found</p>
                    </div>
                ) : (
                    <div className="d-flex flex-column gap-3">
                        {comments.map((item) => (
                            <div key={item.comment._id} className="bg-white rounded shadow-sm p-3">
                                <div className="d-flex gap-3">
                                    <FaUserCircle size={32} color="var(--green)" className="flex-shrink-0 mt-1" />
                                    <div className="w-100">
                                        ...
                                        <div className="d-flex align-items-start justify-content-between gap-2">
                                            <div>
                                                ...
                                                <small className="d-block" style={{ color: "var(--gray)", fontSize: "11px" }}>
                                                    {item.commenter?.email}
                                                </small>
                                            </div>
                                            <div className="d-flex align-items-center gap-3 flex-shrink-0">
                                                <small style={{ color: "var(--gray)", fontSize: "11px" }}>
                                                    <FaClock size={9} /> {formatDate(item.comment.createdAt)}
                                                </small>
                                                <button
                                        ...
                                        {/* COMMENT TEXT */}
                                        <p className="mt-2 mb-2" style={{ fontSize: "14px", color: "var(--text)" }}>
                                            {item.comment.text}
                                        </p>

                                        {/* POST LINK */}
                                        <div className="d-flex align-items-center gap-1" style={{ fontSize: "12px" }}>
                                            <span style={{ color: "var(--gray)" }}>On:</span>
                                            <Link
                                            ...
                                            >
                                                {item.postTitle} <FaExternalLinkAlt size={9} />
                                            </Link>
                                        </div>
            ...
            {/* DELETE MODAL */}
            <div className="modal fade" id="commentDeleteModal" tabIndex="-1" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content" style={{ borderRadius: "12px", border: "none" }}>
                        <div className="modal-header border-0 pb-0">
                            <h5 className="modal-title fw-bold"><TbTrash /> Delete Comment</h5>
                            <button type="button" className="btn-close" id="closeCommentDeleteModal" data-bs-dismiss="modal" />
                        </div>
                        <div className="modal-body pt-2">
                            ...
                            {deleteTarget && (
                                <div className="p-3 rounded" style={{ backgroundColor: "#f8d7da", border: "1px solid #f5c6cb" }}>
                                    <p className="mb-0" style={{ fontSize: "14px", color: "#721c24" }}>
                                        "{deleteTarget.text}"
                                    </p>
                                </div>
                            )}
